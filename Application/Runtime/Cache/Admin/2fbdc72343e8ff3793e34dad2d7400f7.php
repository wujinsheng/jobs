...
<div id="dialog" style="display:none">  
	<div id="tabs">
	  ...
	  <div id="tabs-1">
			<form id="parentForm" method="post" action="<?php echo u('userclass/addp');?>"  onSubmit="return false;" >  
				父类名： 
	  ...
	  <div id="tabs-2">
			<form id="subForm" method="post" action="<?php echo u('userclass/add');?>" onSubmit="return false;" >  
				子类名：
					<input type="text" name="name"/> 
				<br />		
				所属父类：
...
<tbody>
  ...
  <?php if(is_array($items)): $i = 0; $__LIST__ = $items;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$city): $mod = ($i % 2 );++$i;?><tr data-tt-id="<?php echo ($city["id"]); ?>"<?php if($city["pid"] != '0'): ?>data-tt-parent-id="<?php echo ($city["pid"]); ?>"<?php endif; ?>> 
		...
		<td><?php echo ($city["keyname"]); ?></td>
		<td>
			<?php if($city["pid"] != '0'): ?><a class="btn btn-warning btn-xs" href="<?php echo U('userclass/del?id='.$city['id']);?>" onClick="return confirm('确定要删除此分子类？ ');">删除</a>
			<?php else: ?>
				
				<a class="btn btn-danger btn-xs" href="<?php echo U('userclass/delp?id='.$city['id']);?>" onClick="return confirm('确定要删除此分类及其子类？');">删除</a><?php endif; ?>
		</td>
